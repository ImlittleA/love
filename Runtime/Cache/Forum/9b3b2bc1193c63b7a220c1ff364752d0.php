...
<!--顶部导航之后的钩子，调用公告等 end-->
    ...
    <div>
        <div class="with-padding margin-bottom-15">
            <button onclick="rand()" class="btn btn-default btn-block btn-lg">
                <i class="icon-refresh"></i> <?php echo L('_C_C_');?>
            </button>
        </div>
        <div id="posts_list">
            <?php if(is_array($posts)): $i = 0; $__LIST__ = $posts;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; $user = query_user(array('avatar128','avatar64','nickname','uid','space_url'), $vo['uid']); ?>
<div class="clearfix">
    ...
    <script>
        function rand(){
            toast.showLoading();
            $.post("/love/index.php?s=/forum/index/look.html",function(data){
